...
class Club
{
    ...
    public const DIR = WWW . '/data/club/';
...
SQL;
        ...
        foreach ($lesLignes as &$ligne) {
            $ligne['present'] = isset($ligne['fichier']) && $ligne['fichier'] !== '' && is_file(self::DIR . $ligne['fichier']);
        }
        return $lesLignes;
    }
    ...
    {
        $sql = <<<SQL
           select club.id, club.nom, count(*) as nb
           from club
             left join coureur on coureur.idClub = club.id
           group by club.id, club.nom
           order by club.nom;
SQL;
        $select = new Select();
